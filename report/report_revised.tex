...
Part (c) asks for the minimum number of hidden or internal variables required to ``fully learn'' the constitutive relation. That phrase is not mathematically unique, so an operational criterion was needed. Throughout the hidden-size studies, the minimum acceptable hidden dimension was defined as the smallest $h$ whose validation loss lay within $5\%$ of the best validation loss in the relevant sweep or reference study. Formally, if the best validation loss in the comparison set is $L_{\min}$, then the acceptance threshold is
\begin{equation}
...
\begin{table}[t]
\centering
\caption{$h=0$ ablation for the paper-inspired RNO. Removing both latent state and explicit rate makes the constitutive model fail badly, while restoring the rate channel recovers most of the predictive accuracy even before any hidden state is introduced.}
\label{tab:paper_h0_ablation}
\begin{tabular}{lcccc}
\toprule
Variant & Best val. loss & Test RMSE & Test relative $L^2$ & Test $R^2$ \\
\midrule
Paper RNO, no rate, $h=0$ & $3.98\times 10^{-2}$ & $2.44\times 10^{-1}$ & $7.21\times 10^{-1}$ & $0.475$ \\
Paper RNO, with rate, $h=0$ & $8.74\times 10^{-6}$ & $3.88\times 10^{-3}$ & $1.14\times 10^{-2}$ & $0.999868$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{Threshold-style summary of the hidden-size investigations. The GRU study used the Optuna-best GRU as its reference, while the paper-inspired studies used the best value within each sweep.}
\label{tab:threshold_summary}
\begin{tabular}{lcccc}
\toprule
Study & Hidden grid & Best loss & Threshold limit & Selected hidden \\
\midrule
GRU low-dimensional sweep & $0,1,2,3,4$ & $8.36\times 10^{-7}$ (reference) & $8.78\times 10^{-7}$ & none found in grid \\
Paper RNO, no rate & $0,1,2,3,4,5$ & $1.86\times 10^{-2}$ & $1.95\times 10^{-2}$ & $3$ \\
Paper RNO, with rate & $0,1,2,3,4,5$ & $5.36\times 10^{-6}$ & $5.63\times 10^{-6}$ & $5$ \\
\bottomrule
\end{tabular}
\end{table}
...
Figure~\ref{fig:supp_validation_curves} shows that the baseline RNO follows the same broad validation-loss trend as the best cell-based models, but settles at a higher error floor than the GRU. Figure~\ref{fig:supp_hidden_trajectories} adds two more views of the latent geometry: a PCA projection of one representative hidden-state trajectory and the corresponding time traces. These figures do not change the main conclusions, but they make the redundancy of the learned latent spaces more concrete.

\begin{figure}[t]
\centering
\includegraphics[width=0.82\textwidth]{06_best_models_validation_curve_comparison.png}
\caption{Validation curves for the best GRU, baseline RNO, RNN, and LSTM models. The baseline RNO is competitive but remains consistently above the GRU throughout the later training stages.}
\label{fig:supp_validation_curves}
\end{figure}
...
\begin{subfigure}[t]{0.48\textwidth}
    \centering
    \includegraphics[width=\textwidth]{11_hidden_state_pca_trajectories.png}
    \caption{Representative hidden-state trajectories projected onto the leading PCA directions.}
\end{subfigure}
\hfill
\begin{subfigure}[t]{0.48\textwidth}
    \centering
...
\end{subfigure}
\caption{Supplementary latent-state views. The trajectories are smooth and largely confined to low-dimensional subspaces, reinforcing the PCA evidence from Figure~\ref{fig:hidden_state_geometry}.}
\label{fig:supp_hidden_trajectories}
\end{figure}
